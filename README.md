...
中文散文、评论等普通正文段落优先使用 CSS `text-indent:2em` 实现首行缩进。若目标 API 的实际回读证明确实会清除该属性、段首空白和所有视觉空占位元素，则最终 HTML 可使用两个 U+2800 BRAILLE PATTERN BLANK 字符（`&#10240;&#10240;`）作为有证据约束的兼容回退。不要在段首手打全角空格，也不要用空段落或连续 `<br>` 撑开间距；标题、诗歌、标签和经过设计的开篇导语可不缩进。
