...
中文散文、评论等普通正文段落优先使用 CSS `text-indent:2em` 实现首行缩进；若目标 API 的实际回读会同时清除该属性和段首空白实体，则在最终 HTML 中使用 `<span style="display:inline-block;width:2em;">&nbsp;</span>` 作为经过验证的兼容回退。不要在段首手打全角空格，也不要用空段落或连续 `<br>` 撑开间距；标题、诗歌、标签和经过设计的开篇导语可不缩进。
